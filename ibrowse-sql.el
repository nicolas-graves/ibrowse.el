...
(defun ibrowse-sql--extract-fields (db query varname &optional callback)
...
purposes.  Nothing is done when CALLBACK is not provided."
  (ibrowse-core--file-check db varname)
  (setq ibrowse-sql-candidates nil)
  (with-temp-buffer
    (ibrowse-sql--apply-command db (funcall query) callback t)))

(defun ibrowse-sql--get-candidates (db query varname &optional candidate-format)
  "Build candidates and format them with CANDIDATE-FORMAT.

The candidates are generated by calling `ibrowse-sql--extract-fields'
with arguments DB, QUERY and VARNAME."
