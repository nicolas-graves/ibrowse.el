...
(defun ibrowse-sql--extract-fields (db query varname &optional callback temp-db)
  "QUERY the database DB, and call the CALLBACK function.

VARNAME is used to carry the name of the origin of the call, for debug
purposes.  When TEMP-DB is provided, work with a this copy of the
database.  Nothing is done when CALLBACK is not provided."
  (ibrowse-core--file-check db varname)
  (when temp-db
    (ibrowse-sql--ensure-db db temp-db))
  (setq ibrowse-sql-candidates nil)
  (with-temp-buffer
    (ibrowse-sql--apply-command (if temp-db temp-db db)
                                (funcall query)
                                callback)))
...
with arguments DB, TEMP-DB, QUERY and VARNAME."
  (unless ibrowse-sql-candidates
    ...
    (let ((candidates (ibrowse-sql--extract-fields db query varname
